...
#define CONTROL_BUFSIZE	16384
#define MSG_BUFSIZE	16384
	...
	: player(pl), rsa(r)
{
	...
	if ((controlBuffer = (char *)malloc(CONTROL_BUFSIZE))==NULL)
		throw Error(Error::DRTA_ERR_MEMORY);
	if ((msgBuffer = (char *)malloc(MSG_BUFSIZE))==NULL)
		throw Error(Error::DRTA_ERR_MEMORY);
...
void Receiver::resetAudioStream()
{
	streamLen = 0;
...
void Receiver::resetMsg()
{
	msgLen = 0;
	msgPtr = msgBuffer;
	msg_sync = STREAM_READ_DATA;
}
	...
	if (working)
	{
		...
		if (rlen > 0)
		{
			if( valid_data ){
				...
				total += rlen;
				ledOn(true);
				putData(inputBuffer, rlen , str_num);
			}
		}
		else
		{
...
void Receiver::putData(char *buffer, int len , int str_num)
{
	...
	switch ( str_num ) {
		...
		case -1: // for non multi-stream proto
		case MAPLE_STM_AUDIO:
			debug("audio");
		case MAPLE_STM_SPECIAL:
			debug("special");
		case MAPLE_STM_NEW:
		case MAPLE_STM_MSG:
		...
		break;
		case -2:
			if (msgLen > (MSG_BUFSIZE - len))
			{
				qWarning("Warning: receiver msg buffer overloaded.");
				len = MSG_BUFSIZE - msgLen;
			}
			memcpy(msgBuffer + msgLen, buffer, len);
			msgLen += len; 
		break;

		default:
			debug("Receiver::putDta . should not run here :");
		break;
	};
	if (realtime)
	{
...
void Receiver::processAudioData()
{
...
void Receiver::processOtherData()
{
	while ((msg_sync != STREAM_READ_DATA) && (msg_sync != STREAM_PLAYER_NOT_READY))
	{
		switch (msg_sync)
		{
			case STREAM_OK:
				if (msgLen < (HEADER_SYNC_LEN + 1))
				{
					msg_sync = STREAM_MISSING_DATA;
					break;
				}
			case STREAM_DATA:
				if (strncmp(msgPtr, HEADER_SYNC_STRING, HEADER_SYNC_LEN) != 0)
				{
					msg_sync = STREAM_OUT_OF_SYNC;
#ifdef DRTA_DEBUG
					fprintf(stderr, "OUT OF SYNC (packet # %d. Dump: 0x", packets);
					for(int i=0;i<4;i++)
						fprintf(stderr, "%02x", (unsigned char) msgPtr[i]);
					fprintf(stderr, "\n");
#endif
					break;
				}
				...
				{
					unsigned char packetlen = (unsigned char) msgPtr[HEADER_SYNC_LEN];
					int plen = (int) packetlen;
					if (plen > msgLen)
					{
						msg_sync = STREAM_MISSING_DATA;
					...
					else
					{
						try {
							Packet *p = new Packet(plen);
							PacketHandler::readPacket(p, msgPtr, plen);
							processPacket(p);
							if (msg_sync != STREAM_PLAYER_NOT_READY)
							{
								if (plen < msgLen)
								{
									msgPtr = msgPtr + plen;
			...
			case STREAM_OUT_OF_SYNC:
				...
				else
				{
					...
					} while(msg_sync == STREAM_OUT_OF_SYNC);
				}
				break;
			case STREAM_MISSING_DATA:
				msg_sync = STREAM_READ_DATA;
			case STREAM_READ_DATA:
			case STREAM_PLAYER_NOT_READY:
				memmove(msgBuffer, msgPtr, msgLen);
				msgPtr = msgBuffer;
				break;
		}
	}
